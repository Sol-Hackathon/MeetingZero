...
이번 단계에서 만든 것 — 주제 입력부터 재질문까지의 한 사이클입니다.
...
아직 만들지 않은 것: 최종 결론 확정과 리포트 생성(문서 출력), 이메일/슬랙 발송.
...
| --- | --- | --- |
| `/` | 주최자 | 회의 만들기 |
| `/m/{회의ID}?t={주최자토큰}` | 주최자 | 질문 검토, 링크 공유, 응답 현황, 마감, 정리 결과 |
